...
async function loadDataFromJSON() {
    try {
        // Site ayarları ve kategorileri yükle
        const siteResponse = await fetch('site.json');
        if (!siteResponse.ok) throw new Error('site.json yüklenemedi');
        const siteData = await siteResponse.json();
        appData.siteSettings = siteData.siteSettings;
        appData.categories = siteData.categories;
        
        // Formülleri yükle
        const formulasResponse = await fetch('formul.json');
        if (!formulasResponse.ok) throw new Error('formul.json yüklenemedi');
        appData.formulas = await formulasResponse.json();
        ...
        alert('Veri yüklenirken hata oluştu. Sayfayı yenileyin.');
    }
}

// JSON dosyalarına veri kaydetme
async function saveDataToJSON() {
    try {
        // Site ayarları ve kategorileri kaydet
        const siteData = {
        ...
        await fetch('save-formul.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(appData.formulas)
        });
...
window.addEventListener('DOMContentLoaded', () => {
    // Veriyi JSON dosyalarından yükle
    loadDataFromJSON();
